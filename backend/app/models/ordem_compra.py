...
class OrdemCompra(Base):
    ...
    status = Column(
        Enum(StatusOrdemCompra), nullable=False, default=StatusOrdemCompra.aberto
    )
    criado_em = Column(DateTime(timezone=True), server_default=func.now())

    solicitante = relationship("Usuario", back_populates="ordens_compra")
    itens = relationship(
        "OrdemCompraItem", back_populates="ordem", cascade="all, delete"
    )
    ...
    pendencias_substancias = relationship(
        "PendenciaSubstancia", back_populates="ordem", cascade="all, delete"
    )
    franquias = relationship(
        "FranquiaOrdem", back_populates="ordem", cascade="all, delete"
    )
    historico_status = relationship(
        "OrdemCompraStatusHistorico", back_populates="ordem", cascade="all, delete"
    )
...
class OrdemCompraDetalhes(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    ordem_id = Column(
        Integer, ForeignKey("singulist.ordens_compra.id"), nullable=False, unique=True
    )
...
StatusOrdemCompraLiteral = Literal[
    "aberto", "em_andamento", "em_pendencia", "concluido"
]
