...
class OrdemCompraDetalhesCreate(BaseModel):
    ...
    data_compra: Optional[date] = Field(
        None, description="Data em que a compra foi realizada"
    )
    previsao_chegada: Optional[date] = Field(
        None, description="Previsão de chegada dos produtos"
    )
